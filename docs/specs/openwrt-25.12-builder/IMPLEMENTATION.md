...
- Manual `x86_64` workflow runs restore and save only `dl/` and `.ccache/`. Cache operations are non-blocking; the workflow caps ccache at 2 GiB and reports both cache directory sizes.
