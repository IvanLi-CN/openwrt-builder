...
- `config/components.lock.json` uses official feeds, selected Lean 25.12 directories for Netdata and ZeroTier, package-specific upstreams for the remaining third-party applications, and local ramfree plus x86 compatibility packages. The compatibility recipes package only OpenWrt's in-tree Wangxun NGBE/TXGBE and Intel HDA modules; they add no driver source or kernel patch.
